...
def main():
    ccsds = ccsds123.CCSDS123(image_ordering="BIP", delayed_weight_updates=False)
    ...
    for file in os.listdir(directory):
        ...
        with open(f"{output_folder}/results.txt", "a") as file:
            file.truncate(0)
            file.write("P,cr,loading,predictor,encoder,save_data\n")
        ...
        for P in prediction_bands:
            ...
            with open(f"{output_folder}/results.txt", "a") as file:
                file.write(f"{P},{cr:.3f},{timing['loading']:.3f},{timing['predictor']:.3f},{timing['encoder']:.3f},{timing['save_data']:.3f}\n")

            os.rename(hlm_output_folder + "/z-output-bitstream-enc.bin", f"{output_folder}/{P}.bin")
