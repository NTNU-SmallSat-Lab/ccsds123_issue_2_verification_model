...
class CCSDS123:
    ...
    def compress_image(self):
        ...
        if self.header.entropy_coder_type == hd.EntropyCoderType.SAMPLE_ADAPTIVE:
            self.encoder = sa_enc.SampleAdaptiveEncoder(self.header, self.image_constants, self.predictor.get_predictor_output())
        elif self.header.entropy_coder_type == hd.EntropyCoderType.HYBRID:
            self.encoder = hyb_enc.HybridEncoder(self.header, self.image_constants, self.predictor.get_predictor_output())
            ...
            self.encoder = ba_enc.BlockAdaptiveEncoder(self.header, self.image_constants, self.predictor.get_predictor_output())
