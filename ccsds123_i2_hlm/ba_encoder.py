...
class BlockAdaptiveEncoder:
    ...
    def __init_encoder_constants(self):
        ...
        self.reference_sample_interval = (
            self.header.reference_sample_interval
            + 2**12 * int(self.header.reference_sample_interval == 0)
        )
        id_bits_lower_limit = (
            1
            if self.header.restricted_code_options_flag
            == hd.RestrictedCodeOptionsFlag.RESTRICTED
            else 3
        )
        self.id_bits = max(
            ceil(log2(self.image_constants.dynamic_range_bits)), id_bits_lower_limit
        )
        if (
            self.header.restricted_code_options_flag
            == hd.RestrictedCodeOptionsFlag.RESTRICTED
        ):
            self.max_sample_split_bits = (
                -1 if int(self.image_constants.dynamic_range_bits <= 2) else 1
            )
        elif (
            self.header.restricted_code_options_flag
            == hd.RestrictedCodeOptionsFlag.UNRESTRICTED
        ):
            self.max_sample_split_bits = (
                5 if int(self.image_constants.dynamic_range_bits <= 8) else 13
            )
            self.max_sample_split_bits = (
                self.max_sample_split_bits
                if int(self.image_constants.dynamic_range_bits <= 16)
                else 29
            )
        ...
        if (
            ...
            updates_num = ceil(
                self.header.y_size / 2**self.header.error_update_period_exponent
            )
    ...
    def __init_encoder_arrays(self):
        ...
        values_to_encoder = (
            ...
            + self.periodic_error_update_values_num
        )
        self.blocks = np.zeros(
            (
                values_to_encoder // self.block_size
                + int(values_to_encoder % self.block_size != 0),
                self.block_size,
            ),
            dtype=np.int64,
        )
        self.blocks_shape = self.blocks.shape
        compressors_results_num = (
            1 + self.max_sample_split_bits + 1 + 1
        )  # Second extension, sample splitting + 1 for k=0, no compression
        self.encoding_result = np.full(
            (compressors_results_num), fill_value="", dtype="U4096"
        )
        self.encoding_results = np.full(
            (self.blocks.shape[0], compressors_results_num), fill_value="", dtype="U64"
        )
        self.zero_block_count = np.zeros((self.blocks.shape[0]), dtype=np.int64)

        self.bitstream = bitarray()
        self.bitstream_readable = np.full(
            (self.blocks.shape[0]), fill_value="", dtype="U40"
        )

    def __encode_block(self, num):
        start_of_segment = (
            num % self.reference_sample_interval
        ) % self.segment_size == 0
    ...
    def __encode_no_compression(self, num):
        return "1" * self.id_bits + "".join(
            [
                bin(self.blocks[num][i])[2:].zfill(
    ...
    def __encode_sample_splitting(self, num, k):
        ...
        for i in range(self.block_size):
            zeroes = (
                int(bin(self.blocks[num][i])[2:].zfill(32)[:-k], 2)
                if k != 0
                else self.blocks[num][i]
            )
            if (
                zeroes > self.block_size * self.image_constants.dynamic_range_bits
                or len(fs_codes)
                > self.block_size * self.image_constants.dynamic_range_bits
            ):
                fs_codes += (
                    "0"
                    * 2
    ...
    def run_encoder(self):
        ...
        if self.header.sample_encoding_order == hd.SampleEncodingOrder.BI:
            self.blocks = self.blocks.reshape(
                (self.blocks_shape[0] * self.blocks_shape[1])
            )
            index = 0
            ...
            self.blocks = self.mapped_quantizer_index.transpose(
                2, 0, 1
            )  # Transpose to z,y,x order (BSQ)
            self.blocks = self.blocks.reshape(
            ...
            self.blocks = np.pad(
                self.blocks, (0, padding), mode="constant", constant_values=0
            )
