...
def tables_init():
    for i in range(16):
        data = np.genfromtxt(
            table_folder + "code_" + str(i).zfill(2) + ".txt",
            delimiter=",",
            dtype="U256",
        )
        data = np.pad(
            data,
            ((0, code_table_max_length - data.shape[0]), (0, 0)),
            mode="constant",
            constant_values="Z",
        )
    ...
    for i in range(16):
        data = np.genfromtxt(
            table_folder + "flush_" + str(i).zfill(2) + ".txt",
            delimiter=",",
            dtype="U256",
        )
        data = np.pad(
            data,
            ((0, flush_table_max_length - data.shape[0]), (0, 0)),
            mode="constant",
            constant_values="Z",
        )
